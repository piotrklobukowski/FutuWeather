...
class AnimatedDots: UIView {
    ...
    func loadingAnimation() {
        UIView.animate(withDuration: 5, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 3, options: .repeat, animations: {
            self.rep.sublayers?.forEach({ sublayer in
                
                let animUpDown = CABasicAnimation(keyPath: "position.y")
                
                animUpDown.byValue = (-2 * sublayer.frame.height)
                ...
                sublayer.add(animUpDown, forKey: "position.y")
                
                CATransaction.begin()
                
                guard let animation = sublayer.animation(forKey: "position.y") else { CATransaction.disableActions(); return }
